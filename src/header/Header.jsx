...
import { changeTitle, getTitle } from "../commuication/options/options";
...
const options = [
  { label: "SEGMENTS", href: "SEGMENTS1" },
  { label: "MAP", href: "MAP1" },
  { label: "TECHNOLOGY", href: "TECHNOLOGY1" },
];

function Header() {
  const currentTitle = useSelector(getTitle); // Get current title from Redux store
  ...
  // Use useEffect to log whenever title changes
  useEffect(() => {
    console.log("Title changed:", currentTitle);
    setSelectedOption(currentTitle.payload.options.header.title);
  }, [currentTitle]); // Run this effect whenever currentTitle changes
  ...
  return (
    <header className="Header-contain">
      <nav
        aria-label="Global"
        className="fixed top-0 z-50 w-full text-white p-2  lg:px-24 flex items-center justify-between navbar"
      >
        <div className="flex lg:flex-1 lg:max-w-md">
          <a href="#" className="-m-1.5 p-1.5">
            <span className="sr-only">Your Company</span>
            <p className="company-Name">
              <span className="text-white font-extralight"> OSIEDLE </span>{" "}
              <br /> <strong className="text-white"> MALINOWSKIEGO </strong>
            </p>
          </a>
        </div>
        ...
        <PopoverGroup className="hidden lg:flex">
          {options.map((option) => (
            <Link
              ...
              smooth={true}
              duration={500}
              className={`nav-opt px-4 d leading-6 text-white-300 ${
                selectedOption === option.label ? "nav-opt-underline" : ""
              }`}
            >
              {option.label}
            </Link>
        ...
        >
          ...
          <div className="mobile-menu-buttons ">
            <div className=" my-6 mx-7 flex-1">
              <a
                href="#"
                className="mobile-menu-button-a block py-3  text-center"
              >
                CONTACT US
              </a>
            </div>
